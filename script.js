...
const sidebarPanes = document.getElementById('sidebarPanes');
const sidebarPolar = document.getElementById('sidebarPolar');
const sidebarDirecto = document.getElementById('sidebarDirecto');
const sidebarAddProduct = document.getElementById('sidebarAddProduct');
const sidebarDownloadImage = document.getElementById('sidebarDownloadImage');
...
const sidebarRecargo = document.getElementById('sidebarRecargo');
...
function loadFromLocalStorage() {
    ...
    if (savedShared) hasShared = savedShared === 'true';
...
function exportToPDF() {
    if (!hasShared) {
        showMessage('Debes compartir el pedido en el grupo de coordinación antes de descargarlo.', '#fca5a5', '#7f1d1d');
        return;
    }
    const searchTerm = searchInput.value.toLowerCase();
...
async function shareToWhatsApp() {
    ...
    hasShared = true;
    // Notificación desactivada según solicitud
}
...
sidebarPanes.addEventListener('click', () => {
    applyOrderTypeFilter('panes');
    closeSidebar();
});

sidebarPolar.addEventListener('click', () => {
    applyOrderTypeFilter('polar');
    closeSidebar();
});

sidebarDirecto.addEventListener('click', () => {
    applyOrderTypeFilter('directo');
    closeSidebar();
});
...
applyOrderTypeFilter = function (type) {
    ...
    const idMap = {
        'panes': 'sidebarPanes',
        'polar': 'sidebarPolar',
        'directo': 'sidebarDirecto',
        'recargo': 'sidebarRecargo',
        'all': ''
    };
    updateSidebarActive(idMap[type]);
};
...
sidebarDownloadImage.addEventListener('click', () => {
    closeSidebar();
    exportToImage();
});
...
sidebarRecargo.addEventListener('click', () => {
    applyOrderTypeFilter('recargo');
    closeSidebar();
});
...
confirmResponsablesBtn.addEventListener('click', () => {
    ...
    if (responsables[0] || responsables[1]) {
        // La lógica de animación del botón AGG PRODUCTO ha sido eliminada
    }
});
...
// Abrir modal de responsables al iniciar si no se ha configurado en esta sesión
