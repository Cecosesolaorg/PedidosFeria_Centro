...
function updateUI() {
    ...
    // Hide Hero section when a filter is active to declutter the UI
    if (heroSection) {
        heroSection.classList.toggle('hidden', isAnyFilterActive);
    }

    // Sidebar buttons should always be visible in the new design
    filterSelectedBtn.classList.remove('hidden');
    filterPanesBtn.classList.remove('hidden');
    filterPolarBtn.classList.remove('hidden');
    directOrderBtn.classList.remove('hidden');
    ...
    // Active classes for sidebar items
...
document.querySelectorAll('.sidebar-nav-item').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        // Add active to current if it's a filter
        if (btn.id !== 'openAddProductModalBtn' && !btn.id.includes('download') && !btn.id.includes('export') && !btn.id.includes('share')) {
...
function renderCompanyList() {
    ...
    const html = companyNames.map(company => `
                <div class="company-section w-full glass-card p-4">
                    <button class="company-list-item bg-white/5 border-none text-white hover:bg-white/10" data-company="${company}">${company}</button>
    ...
    if (productsContainer.innerHTML.trim().startsWith('<!--')) {
        ...
        if (productNames.length > 0) {
            ...
            const productsHtml = companyProducts.map(item => {
                ...
                const cardClass = isSelected ? 'product-card selected' : 'product-card';
                return `
                            <div class="${cardClass} flex justify-between items-center" data-id="${item.id}" data-name="${item.name}">
                                <h2 class="product-name text-sm sm:text-lg font-medium text-white">${item.id}. ${item.name}</h2>
                            </div>
                        `;
            }).join('');
            productsContainer.innerHTML = productsHtml;
        } else {
            productsContainer.innerHTML = '<p class="col-span-full text-center text-slate-400 text-lg">No hay productos para esta empresa.</p>';
        }
    }
});
...
const newProductTypeField = document.getElementById('newProductType');
const newProductCompanyField = document.getElementById('newProductCompany');
const companySelectionDivField = document.getElementById('companySelectionDiv');

openAddProductModalBtn.addEventListener('click', () => {
    newProductNameField.value = '';
    newProductTypeField.value = 'normal';
    companySelectionDivField.classList.add('hidden');

    // Llenar el select de empresas
    newProductCompanyField.innerHTML = '';
    Object.keys(companyProductMapping).sort().forEach(company => {
        const option = document.createElement('option');
        option.value = company;
        option.textContent = company;
        newProductCompanyField.appendChild(option);
    });

    showModal(addProductModal);
});

newProductTypeField.addEventListener('change', () => {
    if (newProductTypeField.value === 'directo') {
        companySelectionDivField.classList.remove('hidden');
    } else {
        companySelectionDivField.classList.add('hidden');
    }
});
...
confirmAddProductBtn.addEventListener('click', () => {
    let name = newProductNameField.value.trim();
    if (!name) return;

    const type = newProductTypeField.value;
    ...
    if (type === 'directo') {
        const company = newProductCompanyField.value;
...
// --- Dark Mode Logic (Updated for Red/Black Theme) ---
// Since the theme is already dark, we fix the toggle to persist
const htmlEl = document.documentElement;

const setTheme = (theme) => {
    htmlEl.classList.add('dark'); // Always dark for this theme
};
